...
const getAllEmployees = function (req, res) {
  return employeeModel
    ...
    .catch((error) =>
      res.status(404).send(error).json({ message: "Cannot load employees" })
    );
};
...
const getEmpById = function (req, res) {
  ...
  return employeeModel
    ...
    .catch((error) =>
      res.status(404).send(error).json({ message: "employee id not found" })
    );
};
    ...
    .then((employee) => {
      if (employee) {
        ...
        return employee
          .save()
          .then((employee) =>
            res.status(201).send(employee).json({ message: "employee Updated" })
          )
          .catch((error) =>
            res
              .status(500)
        ...
        res
          .status(404)
          .json({ message: "Error in Updating Employee, Employee is Null" });
      }
    })
    .catch((error) =>
      res
...
const deleteEmployee = function (req, res) {
  const employeeFullName = req.params.employeeFullName;
  console.log(`inside deleteEmployee()  :${employeeFullName}`);

  return employeeModel
    .findOneAndRemove({ fullName: employeeFullName })
    .then((employee) =>
      employee
        ? res.status(201).json({ message: "Employee deleted" })
        : res.status(404).json({ message: "Error in deleting employee" })
    )
    .catch((error) => res.status(500).send("employee id not found"));
};
// const deleteEmployee = function (req, res) {
